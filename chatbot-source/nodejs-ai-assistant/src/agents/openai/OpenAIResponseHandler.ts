...
/**
 * Checks if an error is an OpenAI quota/billing error
 */
...
function formatQuotaErrorMessage(error: unknown): string {
  return "⚠️ OpenAI API quota exceeded. Please check your OpenAI account billing and plan details. Visit https://platform.openai.com/account/billing to manage your account.";
}
...
export class OpenAIResponseHandler {
      ...
      while (!isCompleted) {
        for await (const event of currentStream) {
          ...
          if (event.event === "thread.run.failed") {
            ...
            // Preserve OpenAI error structure if available
            if (event.data.last_error) {
              (error as any).code = event.data.last_error.code;
            }
  ...
  private handleStopGenerating = async (event: Event) => {
    ...
    try {
      ...
      // If it's a quota error, we should still handle it
      if (isQuotaError(e)) {
  ...
  private handleError = async (error: Error | unknown) => {
    ...
    if (isQuota) {
      console.error("Quota/billing error detected. User needs to check their OpenAI account.");
    }
